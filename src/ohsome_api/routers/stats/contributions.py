...
from ohsome_api.service.stats import contributions
...
async def post_contributors_count_as_json(
    ...
    return {
        "result": await contributions.get_contributions_count_columns(
...
async def post_contributors_count_as_csv(
    ...
    return {
        "result": await contributions.get_contributions_count_rows(
            ohsome_filter=parameters.ohsome_filter,
            start=parameters.time.start,
            end=parameters.time.end,
            bin_size=parameters.time.bin_size,
