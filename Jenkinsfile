pipeline {
  ...
  stages {
    ...
    stage ('encourage') {
      when { equals expected: 0  , actual: env.BUILD_NUMBER.toInteger()%10}
      steps {
        rocketSend channel: 'jenkinsohsome', message: "Still happily deploying example-artifacts! Keep it up!" , rawMessage: true
      }
    }
    ...
    stage ('reports and statistics'){
      steps {
        script{
          projver=sh(returnStdout: true, script: 'mvn org.apache.maven.plugins:maven-help-plugin:2.1.1:evaluate -Dexpression=project.version | grep -Ev "(^\\[|Download\\w+:)"').trim()
          report_dir="/srv/reports/" + reponame + "/" + projver + "/"
    ...
    stage ('Report status change'){
      when {
        expression {
          return currentBuild.getPreviousBuild().result == 'FAILURE'
        }
      }
      steps {
